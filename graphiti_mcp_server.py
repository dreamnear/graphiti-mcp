#!/usr/bin/env python3
"""
Graphiti MCP Server - Standalone Model Context Protocol server for Graphiti knowledge graphs
"""
...
class GroupIDMiddleware(Middleware):
    """Middleware to extract group_id from HTTP query parameters and store in context.

    Supports both HTTP and SSE transports by extracting group_id from URL query parameters.
    For STDIO transport, this middleware has no effect and passes through normally.
    """

    def __init__(self):
        self.session_group_ids = {}  # Store group_id per session

    async def on_message(self, context: MiddlewareContext, call_next):
        """Extract group_id from HTTP/SSE request and set in context."""
        try:
            # Try to get HTTP request using the dependency function
            # This works for both HTTP and SSE transports
            request = get_http_request()

            # Extract group_id from query parameters
            if hasattr(request, 'query_params') and 'group_id' in request.query_params:
                session_group_id = request.query_params['group_id']

                # Set in context variable for this request
                token = current_group_id.set(session_group_id)

                logger.info(f"Set session group_id from URL parameter: {session_group_id}")
            ...
            # Not an HTTP/SSE request (likely STDIO), proceed normally
            pass

        # No group_id found or not HTTP/SSE request, proceed with default
        return await call_next(context)
...
# Add middleware to handle group_id from URL parameters
mcp.add_middleware(GroupIDMiddleware())
...
async def initialize_server() -> MCPConfig:
    ...
    parser.add_argument(
        ...
        default='stdio',
        help='Transport to use for communication with the client. stdio for local, http/sse for remote (default: stdio)',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--path',
        default=None,
        help='Path for transport endpoint (default: /mcp for HTTP, /sse for SSE)',
    )

    args = parser.parse_args()

    # Set default path based on transport type if not explicitly provided
    if args.path is None:
        if args.transport == 'sse':
            ...
            args.path = '/mcp'  # Not used for stdio, but set for consistency
...
async def run_mcp_server():
    ...
    if mcp_config.transport == 'stdio':
        ...
        logger.info(
            f'Running MCP server with Streamable HTTP transport on {mcp_config.host}:{mcp_config.port}{mcp_config.path}'
        )
        await mcp.run_async(
            transport="http",
        ...
        logger.info(
            f'Running MCP server with SSE transport on {mcp_config.host}:{mcp_config.port}{mcp_config.path}'
        )
        await mcp.run_async(
            transport="sse",
            host=mcp_config.host,
